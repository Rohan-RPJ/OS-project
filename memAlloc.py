...
def main():
	print("1.docx 2.xlsx")
	ext = input("Enter file ext :")

	process_size = []
	blocks_size = []	
	
	if ext == "1":
		temp_list=readinput.read_doc(os.path.join(os.getcwd(),'MINI PROJECT INPUT FILES',"memory allocation strategies.docx"))
		temp_p = temp_list[0].split('\t')
		temp_b = temp_list[1].split('\t')
		#print(temp_b)
		for p in temp_p:
			if p.isdigit():
				process_size.append(int(p))
		
		for b in temp_b:
			if b.isdigit():
				blocks_size.append(int(b))
